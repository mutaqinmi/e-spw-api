...
export const getSiswa = async (nis: number) : Promise<Array<any>> => {
...
export const updateToko = async (id_toko: string, deskripsi_toko: string) => {
...
export const updateBannerToko = async (id_toko: string, banner_toko: string) => {
    return await db.update(table.toko).set({ banner_toko: banner_toko })
        .where(eq(table.toko.id_toko, id_toko));
}
...
export const searchProduk = async (keywords: string) : Promise<Array<any>> => {
    return await db.select().from(table.produk)
        .leftJoin(table.toko, eq(table.produk.id_toko, table.toko.id_toko))
...
export const updateProduk = async (id_produk: string, nama_produk: string, harga: string, stok: number, deskripsi_produk: string, detail_produk: string) => {
    return await db.update(table.produk).set({ nama_produk: nama_produk, harga: harga, stok: stok, deskripsi_produk: deskripsi_produk, detail_produk: detail_produk })
        .where(eq(table.produk.id_produk, id_produk));
}
...
export const searchToko = async (keywords: string) : Promise<Array<any>> => {
    return await db.select().from(table.toko)
        .leftJoin(table.kelas, eq(table.toko.id_kelas, table.kelas.id_kelas))
...
export const getKelompok = async (nis: number) : Promise<Array<any>> => {
    return await db.select().from(table.kelompok)
        .leftJoin(table.siswa, eq(table.kelompok.nis, table.siswa.nis))
        .leftJoin(table.toko, eq(table.kelompok.id_toko, table.toko.id_toko))
        .leftJoin(table.kelas, eq(table.siswa.kelas, table.kelas.id_kelas))
...
export const addToKeranjang = async (id_produk: string, nis: number, jumlah: number, catatan: string) => {
...
export const getNotifikasiToko = async (nis: number, type: string) : Promise<Array<any>> => {
    return await db.select().from(table.notifikasi_toko)
        .where(and(eq(table.notifikasi.nis, nis), eq(table.notifikasi.jenis_notifikasi, type)))
        .orderBy(desc(table.notifikasi.waktu));
}

export const createNotifikasiToko = async (id_toko: string, jenis_notifikasi: string, judul_notifikasi: string, detail_notifikasi: string) => {
...
export const getRiwayatUlasanByToko = async (id_toko: string) : Promise<Array<any>> => {
    return await db.select().from(table.ulasan)
        .leftJoin(table.transaksi, eq(table.ulasan.id_transaksi, table.transaksi.id_transaksi))
        .leftJoin(table.produk, eq(table.ulasan.id_produk, table.produk.id_produk))
        .leftJoin(table.toko, eq(table.produk.id_toko, table.toko.id_toko))
...
export const getRiwayatUlasanByTokoLimited = async (id_toko: string) : Promise<Array<any>> => {
    return await db.select().from(table.ulasan)
        .leftJoin(table.transaksi, eq(table.ulasan.id_transaksi, table.transaksi.id_transaksi))
        .leftJoin(table.produk, eq(table.ulasan.id_produk, table.produk.id_produk))
        .leftJoin(table.toko, eq(table.produk.id_toko, table.toko.id_toko))
...
export const addToFavorit = async (id_toko: string, nis: number) => {
...
export const deleteFromFavorit = async (id_toko: string, nis: number) => {
...
export const updatePassword = async (nis: number, password: string) => {
